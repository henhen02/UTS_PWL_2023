...
const RincianBarang = () => {
  ...
  const data = dataDump.find((item) => item.produk_id === parseInt(id));
  return (
    <>
      {/* {isLoading ? (
        <Main>
      ...
      <Main>
        <div className="py-10 flex flex-col items-center justify-center">
          <img
            src={data?.produk_image}
            alt={`${data?.produk_title}.png`}
            className="w-64 h-64 object-cover rounded-lg shadow-lg mb-4"
          />
          <h1 className="text-3xl font-bold text-gray-800 mb-2">
            {data?.produk_title}
          ...
          <div className="flex flex-col items-center justify-center mb-4">
            ...
            <p className="text-2xl font-bold text-green-600">
              {new Intl.NumberFormat("id-ID", {
                style: "currency",
                currency: "IDR",
              }).format(data?.produk_price)}
            </p>
          </div>
          {/* <div className="flex flex-col items-center justify-center mb-4">
            <h3 className="text-lg font-medium text-gray-700 mb-2">Penjual:</h3>
            <p className="text-lg font-normal text-gray-700 mb-1">
              {data?.data}
            </p>
            <p className="text-lg font-normal text-gray-700 mb-1">
